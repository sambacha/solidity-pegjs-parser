// Conformance Solidity Testfile
// from https://github.com/ethereum/solidity/releases/tag/v0.4.0
...
contract C {
  uint248 x; // @data-types 31 bytes: slot 0, offset 0
  uint16 y; // @data-types 2 bytes: slot 1, offset 0 (does not fit in slot 0)
  uint240 z; // @data-types 30 bytes: slot 1, offset 2 bytes
  uint8 a; // @data-types 1 byte: slot 2, offset 0 bytes
  struct S {
    uint8 a; // @data-types 1 byte, slot +0, offset 0 bytes
    uint256 b; // @data-types 32 bytes, slot +1, offset 0 bytes (does not fit)
  }
  S structData; // @data-types 2 slots, slot 3, offset 0 bytes (does not really apply)
  uint8 alpha; // @data-types 1 byte, slot 4 (start new slot after struct)
  uint16[3] beta; // @data-types 3*16 bytes, slots 5+6 (start new slot for array)
  uint8 gamma; // @data-types 1 byte, slot 7 (start new slot after array)
}
...
library IntegerSet
{
  struct data
  {
    /// @struct-data Mapping item => index (or zero if not present)
    mapping(uint => uint) index;
    /// @struct-data Items by index (index 0 is invalid), items with index[item] == 0 are invalid.
    uint[] items;
    /// @struct-data Number of stored items.
    uint size;
  }
  struct Request {
   bytes calldata data;
...
/// @contract How to use it:
contract User
{
  /// @contract Just a struct holding our data.
  IntegerSet.data data;
  /// @contract Insert something
  function insert(uint v) returns (uint size)
  {
    ///  @contract  Sends `data` via reference, so IntegerSet can modify it.
    IntegerSet.insert(data, v);
    ///  @contract  We can access members of the struct - but we should take care not to mess with them.
    return data.size;
  }
  ///  @contract  Computes the sum of all stored data.
...
// @contract   This broke it at one point (namely the modifiers).
contract DualIndex {
  mapping(uint => mapping(uint => uint)) data;
  address public admin;

  //  @contract  Placeholder statement (both with & without semicolon should be parsed)
  ...
  function set(uint key1, uint key2, uint value) restricted {
    uint[2][4] memory defaults; // @memory "memory" broke things at one time.
...
/**
* This empty function, as well as the commented
